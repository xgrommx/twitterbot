...
(function(ng, $) {
  var app;
  app = ng.module('app', ['app.TweetCtrl', 'app.TweetAnimation', 'app.TweetDirective', 'app.TweetService', 'app.TweetSpinner', 'btford.socket-io', 'ngAnimate', 'ngResource', 'google-maps', 'nvd3ChartDirectives', 'tools']);
  app.constant({
    socketUrl: 'http://localhost'
  });
  app.config(function(socketProvider, socketUrl) {
    var appSocket;
    appSocket = io.connect(socketUrl);
    socketProvider.ioSocket(appSocket);
  });
  ng.module('app.TweetSpinner', []).constant('_START_REQUEST_', '_START_REQUEST_').constant('_END_REQUEST_', '_END_REQUEST_').config([
    '$httpProvider', '_START_REQUEST_', '_END_REQUEST_', function($httpProvider, _START_REQUEST_, _END_REQUEST_) {
      var interceptor;
      interceptor = [
  ...
  ng.module('app.TweetService', []).factory('TweetService', function($resource) {
  ...
  ng.module('app.TweetCtrl', []).controller('TweetCtrl', [
    '$scope', 'socket', 'TweetService', '$log', '$timeout', function($scope, socket, TweetService, $log, $timeout) {
      ...
      TweetService.getTweets({
        hashtag: $scope.message
      }, function(data) {
        $scope.tweets = data.tweets;
        $scope.streamOneLength = $scope.tweets.length;
        $scope.vips = $scope.tweets.where(function(n) {
          return n.followers > 20;
        }).toArray();
      });
      $scope.$watchCollection('vips', function(oldValue, newValue) {
        $scope.groupVips = $scope.vips.groupBy(function(vip) {
          return vip.id;
        }, function(vip) {
          return vip;
        }, function(id, enumObj) {
      ...
      $scope.$watch('message', function(oldVal, newVal) {
        ...
        $scope.streamTwoLength = 0;
        $scope.streamOneLength = 0;
        socket.emit('get:tweet', {
          message: $scope.message
        });
      });
      socket.on('send:vip', function(data) {
        $scope.vips.push({
  ...
  ng.module('app.TweetAnimation', []).animation('.tweet-animation', function() {
    return {
  ...
  ng.module('app.TweetDirective', []).directive('tab', function() {
    return {
      restrict: 'A',
